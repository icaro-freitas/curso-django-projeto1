...
class AuthorsRegisterTest(AuthorsBaseTest):
    ...
    def test_empty_username_error_message(self):
        def callback(form):
            ...
            self.assertIn(
                'This field must not be empty', form.text)
        self.form_field_test_with_callback(callback)

    def test_invalid_email_error_message(self):
        def callback(form):
            email_field = self.get_by_placeholder(form, 'Your username')
            ...
            self.assertIn(
                'The e-mail must be valid', form.text)
        self.form_field_test_with_callback(callback)
    ...
    def test_password_do_not_match(self):
        def callback(form):
            ...
            self.assertIn(
                'Password and password2 must be equal', form.text)
        self.form_field_test_with_callback(callback)
